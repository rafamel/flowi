...
class KeyFlow extends Flow {
    ...
    require(arrOrBool, n = 0) {
        let { error } = Joi.validate(n, Joi.number().integer().min(0));
        if (error) throw Error('The number of keys to require is not a positive integer.');
        this._define.nRequire = n;
    ...
    _validate(toValidate, { noAsync, strip } = {}) {
        ...
        const nRequire = this._define.nRequire;
        if (nRequire === 0) {
            ...
            for (let key of toRequire) {
                if (Object.prototype.hasOwnProperty.call(toValidate, key)) {
                    count++;
                    if (count >= nRequire) break;
                }
            }
            if (count < nRequire) {
                const note = (nRequire === 1)
                    ? `At least one key out of ${String(toRequire)} is required.`
                    : `At least ${nRequire} keys out of ${String(toRequire)} are required.`;
                return {
                    error: new ValidationError(note, { note: note }),
                    value: toValidate
                };
            }
        }
